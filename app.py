...
def generate_explanation(model_type, r2, std, trust_score):
    ...
    if model_type == "선형회귀":
        ...
        parts.append("- 과거 유사 사례를 활용하기 때문에 직관적이며 이해가 쉬운 방식입니다.")
    ...
    if model_type in ["선형회귀", "랜덤포레스트"]:
        if std <= 3:
            parts.append(f"- 예측값과 실제값 간 오차의 표준편차가 **{std:.1f}개월**로 안정적입니다.")
        elif std <= 6:
            parts.append(f"- 예측 오차의 표준편차가 **{std:.1f}개월**로 중간 수준입니다.")
        else:
            parts.append(f"- 예측 오차의 표준편차가 **{std:.1f}개월**로 예측 신뢰도가 낮을 수 있습니다.")
...
top_n = 10  # 상위 10개 변수만 출력 (원하면 변경 가능)

fig_imp, ax_imp = plt.subplots(figsize=(8, 5))
ax_imp.barh(
    y=np.array(feature_names)[sorted_idx[:top_n]][::-1],  # 역순으로 위에서 중요도 높은 항목 표시
